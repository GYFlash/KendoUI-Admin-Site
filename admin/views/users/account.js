$(function () {
    ...
    $.fn.ajaxPost({
        ...
        succeed: function (res) {
            ...
            // 旅游足迹
            var tourismValue = [];
            for (var i = 0; i < res.data[0].tourism.length; i++) {
                tourismValue.push(res.data[0].tourism[i].code);
            }
            ...
            $('#photo').kendoUpload({
                ...
                files: [
                    {
                        name: res.data[0].photo.name + res.data[0].photo.extension,
                        size: res.data[0].photo.size,
                        extension: res.data[0].photo.extension
                    }
                ],
                validation: {
                ...
                success: function (e) {
                    if (e.response.result === 'y') {
                        if (e.operation === 'upload') {
                            $('#photoUrl').val(e.response.data.url);
                            $('picture').attr('style', 'background-image: url(' + e.response.data.url + ');').prev().attr('src', e.response.data.url);
                            alertMsg(e.response.msg, 'success');
                        }
                        if (e.operation === 'remove') {
                            $('#photoUrl').val('');
                            $('picture').attr('style', 'background-image: url(img/avatar.png);').prev().attr('src', 'img/avatar.png');
                        }
                    } else {
            ...
            // 表单验证
            var validator = $('#account form').kendoValidator({
                rules: {
                    // 昵称
            ...
            $('#submitBtn').unbind('click').click(function () {
                if (validator.validate()) {
                    ...
                    noticeMsg('开始提交表单……', 'success', 'center', 500, function () {
                        $('#loading').show();
                        $.fn.ajaxPost({
                            ajaxData: $('#account form').serializeObject(),
                    ...
                    noticeMsg('表单中有选项未填写正确！请检查……', 'error', 'center', 2000, function () {
                        // 出错定位
                        $('.k-invalid-msg:visible').first().parents('p').focus();
                    });
                }
            });
        },
